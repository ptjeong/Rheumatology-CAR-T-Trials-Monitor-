...
with tab_overview:
    ...
    with left:
        ...
        if not counts_phase.empty:
            st.plotly_chart(make_bar(counts_phase, "Phase", "Count", color="#d38a5a"), use_container_width=True)
        else:
            st.info("No trials for the current filter selection.")
        st.markdown("</div>", unsafe_allow_html=True)
...
with tab_geo:
    ...
    if germany_open_sites.empty:
        ...
        with c2:
            ...
            st.dataframe(germany_city_counts, use_container_width=True, height=380, hide_index=True)
    st.markdown("</div>", unsafe_allow_html=True)
