...
**Findings.** Two hundred eighty-four CAR-T autoimmune trials were included after exclusion of 78 oncology, indication-mismatch, and curated false-positive records. Trial initiation grew from a single trial in 2019 to 101 in 2025 (full year) and 41 in the first four months of 2026. Connective tissue disease (n=104, 36.6%) and basket/multidisease platforms (n=98, 34.5%) co-dominated; SLE alone accounted for 79 dedicated trials (27.8% of all trials) and was named as an eligible indication in 141 trials (49.6%) when basket platforms were included. Neurologic autoimmune disease was the largest non-rheumatic disease cluster (n=43, 15.1%), led by myasthenia gravis (n=18) rather than multiple sclerosis (n=11). Dedicated single-disease trials in rheumatoid arthritis (n=3), AAV (n=5), and Sjögren disease (n=1) were scarce, but each indication was named as eligible in many more trials when basket platforms were counted (RA in 17, AAV in 22, Sjögren in 25); spondyloarthritis indications (RA-distinct) were absent from the pipeline. CD19 monotherapy (n=128, 45.1%) and CD19/BCMA dual-targeting (n=76, 26.8%) accounted for 71.8% of trials. Allogeneic and autologous platforms reached parity in 2025-initiated trials (allogeneic 41.6%, autologous 48.5%); in-vivo CAR emerged in 2024 and rose to 12.2% of 2026-initiated trials. China hosted 178 of 256 country-attributed trials; the rest of East Asia, North America, and Europe followed at lower density.
...
The pipeline emits a per-trial dataframe with NCT identifier, brief title, conditions, interventions, brief summary, primary endpoints, lead sponsor and sponsor sector (Academic, Industry, Government, Other), phase, overall status, start year, last-update date, enrollment count, age group, countries (pipe-delimited), and the classification fields (disease entity, target category, product type, sponsor type, classification confidence, trial design [single-disease vs basket/multidisease]). For each trial we record both `DiseaseEntity` (the primary classification, used for single-disease attribution) and `DiseaseEntities` (the pipe-delimited list of *all* indications that the trial registry text matched, including those named within basket platforms). A second site-level dataframe records per-facility country, city, geocoded latitude and longitude, and recruitment status. Each export carries a `#`-prefixed provenance header with the snapshot date, applied filter state, included row count, and source URL.

We report disease coverage in two complementary views. **Dedicated-trial counts** (the modal view in Section 3.2) use `DiseaseEntity` as the primary single-disease classification and assign basket trials to a Basket/Multidisease bucket; this view answers the question *"how many trials are run for disease X as the primary indication?"* **Any-mention counts** (Section 3.2 ad fin., Table 3) use `DiseaseEntities` and tally every trial in which a given indication is named as eligible, including basket platforms; this view answers the question *"how many trials would enroll a patient with disease X?"* The two views differ substantially for indications that the field treats predominantly through basket platforms rather than dedicated single-disease programmes.
...
Two L1 families were strikingly thin under the dedicated-trial view. Vasculitis was represented by only 5 dedicated trials (1.8%; AAV and Behçet combined). Inflammatory arthritis (RA) was represented by 3 dedicated trials (1.1%), of which two were CAR-Treg studies. Spondyloarthritis indications (ankylosing spondylitis, axial spondyloarthritis, psoriatic arthritis) were absent from the cohort altogether — neither dedicated nor named in any basket platform.

When basket platforms are counted alongside dedicated trials (the any-mention view; Table 3), enrollment opportunities for several rheumatologic indications are substantially broader. SLE was named as eligible in 141 trials (49.6% of the cohort; 1.8-fold the dedicated count). SSc was named in 69 (7.7-fold), IIM in 52 (4.7-fold), Sjögren disease in 25 (25-fold), AAV in 22 (4.4-fold), and RA in 17 (5.7-fold). The disparity between dedicated and any-mention counts indicates that the field treats less-frequent rheumatologic indications predominantly through industry-sponsored basket/multidisease platforms rather than through dedicated single-disease programmes. We therefore distinguish between *gaps in dedicated programmes* (a regulatory-pathway concern, since indication-specific labels generally require dedicated trials) and *gaps in patient access to investigational therapy* (a clinical-trial-availability concern). Only the former is acute for RA, AAV, Sjögren disease, and IIM; the latter is largely mitigated by the heavy basket presence. Genuine indication absence — neither dedicated nor basket-mention — applies to spondyloarthritis indications, type 1 diabetes (3 trials, all basket-mention), and most dermatologic autoimmune conditions outside pemphigus.
...
Two distinct gap patterns emerge once dedicated and any-mention views are considered side by side.

The **dedicated-programme gap** affects rheumatoid arthritis (3 dedicated trials, of which 2 are CAR-Treg), AAV (5 dedicated), Sjögren disease (1), and IgG4-related disease (2). The any-mention counts are substantially higher for each of these — RA in 17 trials, AAV in 22, Sjögren in 25, IgG4-RD in 9 — indicating that the field treats these indications via industry-sponsored basket platforms rather than through indication-specific programmes. The clinical-access concern is therefore more limited than the dedicated counts alone suggest: a refractory RA, AAV, or Sjögren patient currently has multiple investigational basket trials potentially open to them. The remaining and arguably more important concern is regulatory: indication-specific approvals generally require dedicated trials with the specific indication as the primary endpoint stratum, and the present pipeline configuration risks generating efficacy signals across many indications without delivering label-grade evidence for any of the under-represented ones. The under-representation of dedicated RA programmes is in part explainable — existing biological and small-molecule DMARDs deliver high response rates and the regulatory bar for CAR-T as a deep-rather-than-broad B-cell intervention is unclear in a disease where response, not refractoriness, is the modal outcome. The under-representation of dedicated AAV programmes is harder to defend on these grounds, since severe and relapsing AAV resembles SLE in its dependence on long-lived autoreactive B-cell clones and the scarcity of treatment options at the refractory end of the spectrum [vandenBrand2023].

The **genuine-absence gap** is smaller but more clear-cut: the spondyloarthritis indications (ankylosing spondylitis, axial spondyloarthritis, psoriatic arthritis) are absent from the cohort altogether, with no dedicated trials and no basket-mention. Type 1 diabetes is covered only in basket platforms (n=3 mentions). Most dermatologic autoimmune conditions outside pemphigus and bullous pemphigoid are absent. The absence of spondyloarthritis is mechanistically defensible — the disease is principally driven by IL-17/IL-23 axis biology rather than by autoreactive B cells, and CAR-T directed at B-cell antigens has limited theoretical rationale here. The absence of pipeline activity for type 1 diabetes (despite a clear autoreactive cellular pathology) and most dermatologic autoimmunity (despite well-defined autoantibody targets in pemphigus, where CAAR-T platforms are emerging) is more notable as a research-priority gap.
...
**Table 1.** *Trial counts by L1 disease family, with L2 detail, recruiting status, and total enrollment.* See [`tables/table1_family_counts.csv`](tables/table1_family_counts.csv).

**Table 2.** *Top 10 sponsors and top 15 named CAR-T products, with target, modality, and lead indication.* See [`tables/table2a_top_sponsors.csv`](tables/table2a_top_sponsors.csv) and [`tables/table2b_top_products.csv`](tables/table2b_top_products.csv).

**Table 3.** *Disease coverage — dedicated single-disease trials versus any-mention (single + basket platforms naming the indication).* See [`tables/table_any_mention_coverage.csv`](tables/table_any_mention_coverage.csv). Highlights: SLE 79/141 (1.8×), SSc 9/69 (7.7×), IIM 11/52 (4.7×), Sjögren 1/25 (25×), AAV 5/22 (4.4×), RA 3/17 (5.7×).
