...
## S2.0 Indications absent from the pipeline

The classification pipeline at commit `540b9c1` does not include patterns for
the following indications, which are therefore neither dedicated nor
basket-mention captured in the analysis:

- **Spondyloarthritis** (ankylosing spondylitis, axial spondyloarthritis,
  psoriatic arthritis) — confirmed absent at 2026-04-25 by both
  pipeline and free-text scan; the absence is mechanistically consistent
  with the IL-17/IL-23 axis biology of these diseases, but the pipeline
  would not detect a CAR-T spondyloarthritis trial if one were registered.
- **Reactive and crystal arthropathies** — out of scope for B-cell-directed
  cell therapy; absence is expected.
- Most **dermatologic autoimmune** conditions outside pemphigus and bullous
  pemphigoid — partial coverage only via the dermatologic-autoimmune L2
  bucket inside Other autoimmune.

Curation queue: an `add_pattern` ticket for spondyloarthritis indications
should be opened if a CAR-T spondyloarthritis trial is registered in a
future snapshot.
...
| `tables/table1_family_counts.csv` | 30 | 5 | L1 family with L2 detail, recruiting status, and total enrollment. |
| `tables/table2a_top_sponsors.csv` | 10 | 5 | Top 10 lead sponsors by trial count. |
| `tables/table2b_top_products.csv` | 15 | 5 | Top 15 named CAR-T products with target, modality, leading indication. |
| `tables/table_any_mention_coverage.csv` | 12 | 5 | Disease coverage: dedicated vs any-mention (single + basket platforms naming the indication). |
